...
export async function middleware(request: NextRequest) {
  ...
  if (!url || !key) { const loginUrl = request.nextUrl.clone(); loginUrl.pathname = "/login"; loginUrl.searchParams.set("error", "El acceso interno requiere configurar Supabase"); return NextResponse.redirect(loginUrl); }
  ...
  if (!profile?.is_active || profile.role === "CONSULTA_PUBLICA") { const denied = request.nextUrl.clone(); denied.pathname = "/no-autorizado"; return NextResponse.redirect(denied); }
  return response;
}
