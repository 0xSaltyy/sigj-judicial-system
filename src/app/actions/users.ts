...
export async function inviteUser(formData: FormData) {
  ...
  if (!process.env.NEXT_PUBLIC_APP_URL) usersRedirect("error", "Falta NEXT_PUBLIC_APP_URL", "/admin/usuarios/nuevo");
  ...
  try {
    await writeAudit(supabase, user.id, data.user.id, "USER_INVITED", "Usuario creado mediante invitación del propietario", null, { role: profile.role, dependency_id: profile.dependency_id, is_active: true });
  } catch (error) {
    usersRedirect("error", error instanceof Error ? error.message : "No fue posible registrar la auditoría", "/admin/usuarios/nuevo");
  }
  revalidatePath("/admin/usuarios");
  usersRedirect("success", "Invitación enviada correctamente");
}
