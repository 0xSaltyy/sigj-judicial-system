...
export async function login(formData: FormData) {
  ...
  if (!supabase)
    redirect(
      "/login?error=El%20acceso%20interno%20requiere%20configurar%20Supabase",
    );
  const { data, error } = await supabase.auth.signInWithPassword(parsed.data);
  if (error)
    redirect(
      `/login?error=${encodeURIComponent("Credenciales inválidas o usuario inactivo")}`,
    );
...
export async function updatePassword(formData: FormData) {
  ...
  if (!supabase)
    redirect(
      "/actualizar-password?error=Supabase%20no%20est%C3%A1%20configurado",
    );
