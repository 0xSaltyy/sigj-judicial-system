...
export async function GET(request: Request) {
  ...
  const parsed = caseNumber.safeParse(params.get("case_number") || params.get("q") || params.get("radicado"));
  ...
  if (supabase) {
    ...
    return NextResponse.json({ case: { caseNumber: data.case_number || data.internal_number, docketNumber: data.docket_number || null, legacyInternalNumber: data.internal_number, legacyJudicialNumber: data.judicial_number, filingStatus: data.filing_status, court: data.chamber, processType: data.process_type, processSubtype: data.process_subtype, status: data.status, filedAt: data.filed_at }, latest: latest ? { title: latest.title, description: latest.description, date: latest.action_date } : null });
  }
