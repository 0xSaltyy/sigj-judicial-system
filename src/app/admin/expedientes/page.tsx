...
export default async function CasesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; error?: string; success?: string }> }) {
  const { supabase, profile } = await requirePermission({ resource: "expedientes", action: "view" });
  const [query, canCreate, canArchive, canRestore, canHardDelete] = await Promise.all([
    searchParams,
  ...
  return <><AdminPageHeader title="Expedientes judiciales" description="Radicación, asignación y seguimiento con datos reales de Supabase." action={canCreate ? <Button asChild className="gap-2 bg-[#153b5c]"><Link href="/admin/expedientes/nuevo"><Plus className="size-4" /> Nueva radicación</Link></Button> : <Button disabled title="No tiene permiso para crear expedientes"><Plus className="size-4" /> Nueva radicación</Button>} />
