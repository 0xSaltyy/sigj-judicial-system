...
export default async function CasesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; error?: string; success?: string }> }) {
  ...
  let request = supabase.from("cases").select("id,internal_number,judicial_number,title,chamber,status,confidentiality_level,filed_at,claimant_name,defendant_name").order("filed_at", { ascending: false }).limit(100);
