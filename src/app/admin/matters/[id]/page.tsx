  ...
  return (
    <>
      <AdminPageHeader
        ...
        action={<div className="flex flex-wrap gap-2"><Button asChild variant="outline" className="gap-2"><Link href={`/api/roleplay/matters/${matter.id}/pdf`}><FileDown className="size-4" /> Descargar PDF</Link></Button><Button asChild variant="outline" className="gap-2"><Link href={`/admin/matters/${matter.id}/editar`}><Edit3 className="size-4" /> Editar Matter</Link></Button><Button asChild className="gap-2 bg-[#153b5c]"><Link href={`/admin/matters/${matter.id}/abrir-federal-case`}>Abrir Federal Case a partir de este Matter <ArrowRight className="size-4" /></Link></Button><LifecycleActions resource="matters" id={matter.id} archived={Boolean(matter.archived_at)} compact /></div>}
      />
