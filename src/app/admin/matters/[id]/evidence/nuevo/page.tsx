...
export default async function NewEvidenceForMatterPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <>
      ...
      <form id="matter-evidence-upload" action={createEvidenceItemAction}>
        <input type="hidden" name="matter_id" value={matter.id} />
        <Card>
          <CardHeader><CardTitle>Evidence metadata and custody</CardTitle></CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2"><FormRecovery formId="matter-evidence-upload" storageKey={`matter:${matter.id}:evidence-upload`} /></div>
            <label className="flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed bg-slate-50 p-6 text-center transition hover:bg-slate-100 md:col-span-2">
              ...
              <Input name="evidence_file" type="file" required accept=".pdf,.doc,.docx,.txt,.csv,.png,.jpg,.jpeg,.webp,.gif,.mp4,.mov,.webm,.m4v,.mp3,.wav,.m4a,.aac,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,text/csv,image/png,image/jpeg,image/webp,image/gif,video/mp4,video/quicktime,video/webm,video/x-m4v,audio/mpeg,audio/wav,audio/wave,audio/x-wav,audio/mp4,audio/aac,application/octet-stream" className="mt-4 max-w-md bg-white" />
            </label>
