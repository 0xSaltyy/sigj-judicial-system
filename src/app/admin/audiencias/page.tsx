...
export default async function AdminHearingsPage() {
  ...
  return <><AdminPageHeader title="Agenda de audiencias" description="Programación y gestión de sesiones físicas y virtuales." action={<Button className="gap-2 bg-[#153b5c]"><CalendarPlus className="size-4" /> Programar audiencia</Button>} />{rows.length === 0 ? <EmptyState title="No hay audiencias programadas" description="Las audiencias creadas para expedientes aparecerán en esta agenda." icon={<CalendarDays className="size-6" />} /> : <div className="grid gap-4 lg:grid-cols-2">{rows.map((item) => <article key={item.id} className="rounded-lg border bg-white p-5"><div className="flex items-start justify-between"><div className="flex gap-4"><div className="rounded bg-[#153b5c] px-3 py-2 text-center text-white"><p className="mono-number text-xs font-semibold">{formatDateTime(item.scheduled_at)}</p></div><div><p className="text-xs font-semibold uppercase tracking-wider text-[#9a752f]">{item.hearing_type}</p><h2 className="mt-1 text-base font-semibold text-[#153553]">{item.title}</h2><p className="mono-number mt-1 text-xs text-muted-foreground">{Array.isArray(item.cases) ? item.cases[0]?.internal_number : item.cases?.internal_number}</p></div></div><Button variant="ghost" size="icon"><MoreHorizontal className="size-4" /></Button></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t pt-4"><p className="text-xs text-muted-foreground">{item.room} · {item.is_public ? "Pública" : "Reservada"}</p><CaseStatusBadge status={item.status} /></div><div className="mt-4 border-t pt-4"><LifecycleActions resource="hearings" id={item.id} archived={Boolean(item.archived_at)} compact /></div></article>)}</div>}</>;
}
