...
export default async function UserPermissionsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Query> }) {
  ...
  const [{ data: profile }, { data: overrides }, { data: roleRules }, { data: dependencies }] = await Promise.all([
    supabase.from("profiles").select("id,full_name,role,is_active,is_owner,institution_id,dependency_id,position_title,is_dependency_leader").eq("id", id).maybeSingle(),
  ...
  return <>
    ...
    <UserPermissionsEditor userId={profile.id} role={role} overrides={(overrides ?? []) as { resource: string; action: string; effect: "allow" | "deny"; reason: string | null }[]} roleRules={(roleRules ?? []) as { role: AppRole; resource: string; action: string; allowed: boolean }[]} scope={{ institution: dependencies?.find((item) => item.id === profile.institution_id)?.name || "Sin institución", dependency: dependencies?.find((item) => item.id === profile.dependency_id)?.name || "Sin dependencia", position: profile.position_title || "Sin cargo", isLeader: profile.is_dependency_leader }} />}
  </>;
}
