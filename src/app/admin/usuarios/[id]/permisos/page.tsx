...
export default async function UserPermissionsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Query> }) {
  ...
  return <>
    <AdminPageHeader title={`Permisos personalizados · ${profile.full_name}`} description={`${ROLE_DESCRIPTIONS[role].label} · ${profile.is_active ? "Cuenta activa" : "Cuenta inactiva"}`} action={<Button asChild variant="outline"><Link href="/admin/usuarios"><ArrowLeft className="size-4" /> Usuarios</Link></Button>} />
