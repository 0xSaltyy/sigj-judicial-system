...
import { AlertTriangle, ArrowRight, CalendarDays, FileCheck2, FolderKanban, Gavel, Plus, ShieldAlert } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [casesCount, hearingsCount, warrantsCount, applicationsCount, complaintsCount, latestCases, latestHearings, auditLogs] = supabase ? await Promise.all([
    ...
    supabase.from("cases").select("id,internal_number,title,status").is("archived_at", null).order("created_at", { ascending: false }).limit(5),
  ...
  return (
    <>
      <AdminPageHeader title="Panel interno DOJ" description="Resumen operativo basado en datos reales de Supabase. Si no existen registros, los contadores muestran 0." action={<Button asChild className="gap-2 rounded-none bg-[#005ea8] hover:bg-[#1a4480]"><Link href="/admin/expedientes/nuevo"><Plus className="size-4" /> Nuevo expediente</Link></Button>} />
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard label="Expedientes activos" value={String(casesCount?.count ?? 0)} detail="Registros no archivados" icon={<FolderKanban className="size-5" />} />
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="text-base text-[#153553]">Últimos expedientes</CardTitle><Link href="/admin/expedientes" className="text-xs font-semibold text-[#005ea8]">Ver expedientes</Link></CardHeader><CardContent className="divide-y p-0">{caseRows.length === 0 ? <EmptyList text="No hay expedientes registrados." /> : caseRows.map((item) => <Link key={item.id} href={`/admin/expedientes/${item.id}`} className="flex items-center justify-between gap-3 px-6 py-4 hover:bg-slate-50"><div><p className="mono-number text-xs font-semibold text-[#153553]">{item.internal_number}</p><p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{item.title}</p></div><CaseStatusBadge status={item.status} /></Link>)}</CardContent></Card>
