...
type CountTotalRow = { total_valid: number | string | null };
...
export default async function ElectionDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;success?:string}>}){
  ...
  const [{data:election},{data:options},{data:dependencies},{data:countTotals},{data:countBatches},canEdit]=await Promise.all([
  ...
  const totalExpected=Number(election.total_expected_votes??0);
  const counted=((countTotals??[]) as CountTotalRow[]).reduce((sum,row)=>sum+Number(row.total_valid??0),0);
  const inReview=(countBatches??[]).filter((batch)=>batch.status==="submitted").reduce((sum,batch)=>sum+sumCounts(batch.option_counts)+Number(batch.annulled_votes??0)+Number(batch.rejected_votes??0),0);
  const returned=(countBatches??[]).filter((batch)=>batch.status==="returned").length;
  const rejected=(countBatches??[]).filter((batch)=>batch.status==="rejected").length;
  const validatedProgress=totalExpected>0?Math.min(100,(counted/totalExpected)*100):0;
  return <><AdminPageHeader title={election.title} description={`${election.office} · ${statusLabel(ELECTION_STATUS_LABELS,election.status)}`} action={<div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/admin/elecciones">Volver</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/tarjeta`}>Tarjeta</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/escrutinio`}>Escrutinio</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/analisis-votos`}>Análisis votos</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/resultados`}>Conteo electoral</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/actualizaciones`}>Actualizaciones</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/acta`}>Acta</Link></Button><Button asChild variant="outline"><Link href={`/admin/elecciones/${id}/pantalla`} target="_blank">Pantalla</Link></Button><Button asChild variant="outline"><Link href={`/elecciones/${election.slug}`} target="_blank">Pública</Link></Button></div>}/><ActionMessage error={query.error} success={query.success}/>
    {(inReview>0||returned>0||rejected>0||validatedProgress>=100)&&<section className="mb-5 grid gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
      {inReview>0&&<p>Hay votos enviados a revisión pendientes de validación.</p>}
      {returned>0&&<p>Hay lotes devueltos para corrección.</p>}
      {rejected>0&&<p>Existen lotes rechazados en auditoría.</p>}
    ...
    <section className="mb-5 rounded-xl border bg-white p-4">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-4 xl:grid-cols-8">{[
        ...
        ["Conteo electoral",`/admin/elecciones/${id}/resultados`,"Total esperado y votos por tarjeta"],
        ["Escrutinio",`/admin/elecciones/${id}/escrutinio`,"Revisión humana"],
        ["Análisis",`/admin/elecciones/${id}/analisis-votos`,"Señales sospechosas solo lectura"],
        ["Publicación",`/admin/elecciones/${id}/resultados`,"Actualizar/publicar"],
