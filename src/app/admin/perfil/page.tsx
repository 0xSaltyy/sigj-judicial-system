...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <>
    ...
    <Card className="mb-5"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><UserRound className="size-4"/> Identidad y pertenencia institucional</CardTitle></CardHeader><CardContent>
      <form action={updateSelfProfile} className="grid gap-5 md:grid-cols-2">
        <Field label="Nombre visible en el sistema"><Input name="full_name" defaultValue={profile.full_name} required /></Field>
        ...
        <Field label="Cargo / título institucional" helper={!editTitle ? "Para cambiar el cargo, contacte a un administrador autorizado." : undefined}><Input name="position_title" defaultValue={profile.position_title || ""} disabled={!editTitle}/>{!editTitle&&<input type="hidden" name="position_title" value={profile.position_title || ""}/>}</Field>
        ...
        <Field label="Nombre público"><Input name="public_display_name" defaultValue={profile.public_display_name || ""} disabled={!editPublic}/></Field>
        <Field label="Cargo público"><Input name="public_title" defaultValue={profile.public_title || ""} disabled={!editPublic}/></Field>
        <div className="md:col-span-2"><Field label="Biografía pública"><Textarea name="public_bio" defaultValue={profile.public_bio || ""} maxLength={1200} disabled={!editPublic}/></Field></div>
        <Field label="Teléfono u oficina pública"><Input name="public_phone" defaultValue={profile.public_phone || ""} disabled={!editPublic}/></Field>
        ...
        <div className="md:col-span-2"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Vista previa de tarjeta pública</p><div className="flex max-w-lg items-center gap-4 rounded-lg border bg-white p-4">{avatar?<Image src={avatar} alt="Vista previa de foto" width={64} height={64} unoptimized className="size-16 rounded-full object-cover"/>:<div className="grid size-16 place-items-center rounded-full bg-slate-100 text-xs">Sin foto</div>}<div><p className="font-semibold">{publicName}</p><p className="text-sm text-muted-foreground">{publicTitle}</p><p className="text-xs text-muted-foreground">{names.get(profile.public_dependency_id || profile.dependency_id) || names.get(profile.public_institution_id || profile.institution_id) || "Sin ubicación pública"}</p>{profile.public_bio&&<p className="mt-2 text-xs">{profile.public_bio}</p>}</div></div></div>
...
function ControlledSelect({label,name,value,editable,options,empty}:{label:string;name:string;value:string|null;editable:boolean;options:{value:string;label:string}[];empty:string}){return <Field label={label} helper={!editable?"Para cambiar institución o dependencia, contacte a un administrador autorizado.":undefined}><select name={name} defaultValue={value||""} disabled={!editable} className="h-9 w-full rounded-md border px-3 text-sm"><option value="">{empty}</option>{options.map((option)=><option key={option.value} value={option.value}>{option.label}</option>)}</select>{!editable&&<input type="hidden" name={name} value={value||""}/>}</Field>}
