...
export default async function PublicCasesPage() {
  ...
  const { data } = supabase ? await supabase.from("cases").select("id,case_number,internal_number,title,summary,status,chamber,filed_at").eq("public_visibility", true).eq("confidentiality_level", "Público").is("archived_at", null).order("filed_at", { ascending: false }).limit(50) : { data: null };
  const publicCases = data ?? [];
  return (
    <>
      <PageHero eyebrow="Expedientes públicos" title="Casos públicos" description="Listado de casos marcados como públicos. Los reservados no se exponen." />
