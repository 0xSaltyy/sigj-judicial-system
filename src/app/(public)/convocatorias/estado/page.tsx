...
type Result = { tracking_code: string; applicant_name: string; application_type: string; status: string; submitted_at: string; updated_at: string; public_message: string | null };
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-[820px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        ...
        {result ? <section className="mt-6 border bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-4 border-b pb-4"><div><p className="mono-number text-xs font-semibold text-[#005ea8]">{result.tracking_code}</p><h2 className="mt-2 font-serif text-2xl font-semibold text-[#112f4e]">{result.applicant_name}</h2></div><Badge className="rounded-none bg-[#112f4e]">{result.status}</Badge></div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><Info label="Tipo" value={labelType(result.application_type)} /><Info label="Fecha de postulación" value={formatDateTime(result.submitted_at)} /><Info label="Última actualización" value={formatDateTime(result.updated_at)} /><Info label="Mensaje público" value={safeText(result.public_message, "Sin mensaje público adicional.")} /></dl></section> : null}
