...
export function JudicialPrintFooter({ verification }: { verification?: string }) {
  const generatedAt = new Intl.DateTimeFormat("es-CO", { dateStyle: "long", timeStyle: "short" }).format(new Date());
  return <footer className="judicial-print-footer mt-14 border-t pt-4 text-[10px] leading-4 text-slate-500"><div className="flex justify-between gap-6"><p>{verification || "Documento generado por el Sistema Integral de Gestión Judicial."}</p><p className="shrink-0">Generado: {generatedAt}</p></div><p className="print-page-number mt-2 text-right" /></footer>;
}
