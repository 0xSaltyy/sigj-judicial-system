...
type Election={id:string;title:string;office:string;territory:string;period:string;round_label:string;institution_id:string|null;status:string;opens_at:string;closes_at:string;description:string;instructions:string|null;ballot_image_path:string|null};
export function ElectionForm({dependencies,election}:{dependencies:Dependency[];election?:Election}){
  ...
  return <DraftForm action={saveElection} storageKey={`sigj:election:${election?.id??"new"}`} className="grid min-w-0 gap-5 rounded-xl border bg-white p-5 md:grid-cols-2">
    ...
    <Field label="Imagen de tarjeta electoral"><Input name="ballot_image_path" defaultValue={election?.ballot_image_path??"/VOTACIONES/CARTA DE VOTACION.png"}/></Field>
