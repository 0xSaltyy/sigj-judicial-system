...
import { ACTION_LABELS, defaultRoleCan, MANAGEABLE_PERMISSION_CATALOG, permissionKey, USER_PERMISSION_DESCRIPTIONS } from "@/lib/permissions/catalog";
...
export function UserPermissionsEditor({ userId, role, overrides, roleRules, scope }: { userId: string; role: AppRole; overrides: Override[]; roleRules: Rule[]; scope: { institution: string; dependency: string; position: string; isLeader: boolean } }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <Card className="border-blue-200 bg-blue-50"><CardHeader><CardTitle className="text-base">Alcance efectivo</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><div className="flex flex-wrap gap-2"><Badge variant="outline">Institución: {scope.institution}</Badge><Badge variant="outline">Dependencia: {scope.dependency}</Badge><Badge variant="outline">Cargo: {scope.position}</Badge><Badge variant="outline">{scope.isLeader ? "Encargado/Líder" : "No es líder"}</Badge></div><p><strong>Mi dependencia</strong> = despacho, juzgado, sala u oficina asignada. <strong>Mi institución</strong> = corporación superior. <strong>Global</strong> = toda la estructura autorizada.</p><p className="text-xs text-muted-foreground">Crear usuarios requiere “Crear usuarios” y el permiso de alcance correspondiente. Para jueces o magistrados, además exige estar marcado como encargado/líder.</p></CardContent></Card>
    <Card>
