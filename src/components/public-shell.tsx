...
export function InstitutionalHeader() {
  return (
    <header className="no-print">
      <div className="bg-[#0e2943] text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <Link href="/" aria-label="Inicio SIGJ"><InstitutionalMark /></Link>
          <div className="hidden items-center gap-6 text-xs text-slate-300 md:flex">
            <span className="flex items-center gap-2"><ShieldCheck className="size-4 text-[#cdb374]" /> Portal demostrativo no oficial</span>
            <span className="h-5 w-px bg-white/15" />
            <Link href="/login" className="font-semibold text-white hover:text-[#dcc68e]">Acceso institucional</Link>
          </div>
        </div>
      </div>
      <nav className="border-b bg-white shadow-[0_2px_12px_rgba(17,43,70,.05)]" aria-label="Navegación principal">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="hidden h-full items-center gap-7 lg:flex">
            {links.map(([label, href]) => <Link key={href} href={href} className="flex h-full items-center border-b-2 border-transparent text-[13px] font-medium text-slate-600 transition hover:border-[#b38a3c] hover:text-[#112b46]">{label}</Link>)}
          </div>
          <Sheet>
            <SheetTrigger asChild><Button variant="ghost" size="icon" className="lg:hidden" aria-label="Abrir menú"><Menu /></Button></SheetTrigger>
            <SheetContent side="left" className="w-80 bg-[#102d49] p-6 text-white"><SheetTitle className="sr-only">Navegación</SheetTitle><InstitutionalMark />
              <div className="mt-8 grid gap-1">{links.map(([label, href]) => <Link key={href} href={href} className="rounded px-3 py-3 text-sm hover:bg-white/10">{label}</Link>)}</div>
              <Button asChild className="mt-8 w-full bg-[#b38a3c] text-white hover:bg-[#9a752d]"><Link href="/login">Acceso institucional</Link></Button>
            </SheetContent>
          </Sheet>
          <Link href="/consulta" className="ml-auto flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#183d61]"><Search className="size-4" /> Consultar radicado</Link>
        </div>
      </nav>
    </header>
  );
