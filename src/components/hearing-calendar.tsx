...
export type HearingCalendarItem = {
  id:string; title:string; type:string; scheduledAt:string|null; endAt:string|null; status:string;
...
export function HearingCalendar({items}:{items:HearingCalendarItem[]}) {
  const datedItems=useMemo(()=>items.filter((item)=>isValidDate(item.scheduledAt)),[items]);
  const undatedItems=useMemo(()=>items.filter((item)=>!isValidDate(item.scheduledAt)),[items]);
  const [cursor,setCursor]=useState(()=>{const first=datedItems.find((item)=>new Date(item.scheduledAt!)>=startOfToday());return first?new Date(first.scheduledAt!):new Date();});
  ...
  const byDay=useMemo(()=>{const map=new Map<string,HearingCalendarItem[]>();for(const item of datedItems){const key=dayKey(new Date(item.scheduledAt!));map.set(key,[...(map.get(key)??[]),item]);}return map;},[datedItems]);
  const visibleAgenda=[...datedItems.filter((item)=>{const date=new Date(item.scheduledAt!);return date.getFullYear()===cursor.getFullYear()&&date.getMonth()===cursor.getMonth();}),...undatedItems];
...
function dateLabel(value:string|null|undefined){return isValidDate(value)?new Intl.DateTimeFormat("es-CO",{day:"2-digit",month:"short"}).format(new Date(value as string)):"Sin fecha";}
function time(value:string|null|undefined){return isValidDate(value)?new Intl.DateTimeFormat("es-CO",{hour:"2-digit",minute:"2-digit"}).format(new Date(value as string)):"Fecha no definida";}
