...
export function DocumentUploader({
  ...
  hint = "PDF, DOCX, PNG o JPG · máximo 3 MB en total",
  maxTotalBytes = SERVER_ACTION_FILE_MAX_BYTES,
}: {
  name?: string;
  multiple?: boolean;
  required?: boolean;
  ...
  function selectFiles(files: FileList | null) {
    ...
    if (totalBytes > maxTotalBytes) {
      ...
      setPreviews([]);
      setFileError(
        `La selección supera ${formatBytes(maxTotalBytes)}. Quite archivos o radique el expediente sin anexos.`,
      );
      return;
    }
    setFileError(null);
  ...
  function removeFile(target: LocalPreview) {
    URL.revokeObjectURL(target.url);
    const remaining = previews.filter((item) => item !== target);
  ...
  return (
    <div className="space-y-4">
      <label className="flex cursor-pointer flex-col items-center rounded-lg border border-dashed p-8 text-center hover:bg-slate-50">
        <FileUp className="size-7 text-[#416786]" />
        <span className="mt-3 text-sm font-semibold text-[#153553]">
          {label}
        </span>
      ...
      {previews.length > 0 && (
        <div className="grid gap-3 md:grid-cols-2">
          {previews.map((preview) => (
            ...
            >
              <LocalDocumentPreview preview={preview} />
...
function LocalDocumentPreview({ preview }: { preview: LocalPreview }) {
  if (preview.file.type === "application/pdf")
    return (
      <iframe
        title={`Vista previa de ${preview.file.name}`}
        src={preview.url}
  ...
  if (preview.file.type.startsWith("image/"))
    return (
      <div className="grid h-64 place-items-center bg-slate-50 p-3">
        <img
          src={preview.url}
  ...
  return (
    <div className="grid h-40 place-items-center bg-slate-50 text-slate-500">
