-- El bloqueo sólo puede adquirirse sobre registros que el usuario puede editar.
...
begin
  if p_record_type='proceeding' then
    select case_id into v_case_id from public.proceedings where id=p_record_id and archived_at is null;
    ...
    -- Para actas el identificador de bloqueo es el de la audiencia, incluso antes de crear el acta.
...
revoke all on function public.can_lock_record(text,uuid) from public,anon;
grant execute on function public.can_lock_record(text,uuid) to authenticated;
