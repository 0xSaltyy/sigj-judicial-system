-- Fix sealed ballot integrity hashing for Supabase/Postgres pgcrypto signatures.
...
begin
  ...
  values (
    ...
    p_member_id,
    p_ballot_value,
    encode(digest(convert_to(p_round_id::text || p_count_id::text || p_member_id::text || p_ballot_value || now()::text, 'UTF8'), 'sha256'), 'hex')
  )
...
begin
  ...
  values (
    ...
    p_panel_id,
    p_ballot_value,
    encode(digest(convert_to(p_round_id::text || p_question_id::text || p_panel_id::text || p_ballot_value || now()::text, 'UTF8'), 'sha256'), 'hex')
  )
